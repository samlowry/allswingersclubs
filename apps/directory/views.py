...
def state2(request, state_usps_name):
    ...
    try:
        ...
        pass
    clubs = Hookup.current_site_only.select_related('state','city').filter(state__usps_name__exact=state_usps_name).filter(datetime_of_publish__range=[date(1900, 1, 1),date.today()]).order_by('datetime_of_publish').reverse()
...
def hookup(request, hookup_id, hookup_urlsafe_title):
    try:
        current_hookup = Hookup.current_site_only.select_related('state','city').filter(id__exact=hookup_id).filter(datetime_of_publish__range=[date(1900, 1, 1),date.today()]).get()
